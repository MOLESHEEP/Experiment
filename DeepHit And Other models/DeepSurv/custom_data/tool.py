...
def generateH5():
    ...
    csv_file = r'C:\Experiment\DeepHit And Other models\DeepSurv\custom_data\WeibullLinear\weibullLinearData.csv'
    df = pd.read_csv(csv_file)

    train,test = train_test_split(df,test_size=0.3,random_state=1,shuffle=None)
    # 创建一个HDF5文件并将数据写入其中
    with h5py.File(r'C:\Experiment\DeepHit And Other models\DeepSurv\data\weibullLinear\weibullLinear_new_survaival.h5', 'w') as hf:
    ...
    # file_path = r'C:\Experiment\DeepHit And Other models\DeepSurv\data\gaussian\gaussian_survival_data.h5'

...
(x_dim), (data, time, label), (mask3, mask4) = impt.import_dataset('./sample data/LIFELINE/cleaned_features_final.csv','./sample data/LIFELINE/label.csv',norm_mode='standard')
...
for out_itr in range(OUT_ITERATION):
    ...
    (tr_data,te_data, tr_time,te_time, tr_label,te_label, 
     tr_mask1,te_mask1, tr_mask2,te_mask2)  = train_test_split(data, time, label, mask1, mask2, test_size=0.99, random_state=None) 

    (tr_data,va_data, tr_time,va_time, tr_label,va_label, 
     tr_mask1,va_mask1, tr_mask2,va_mask2)  = train_test_split(tr_data, tr_time, tr_label, tr_mask1, tr_mask2, test_size=0.20, random_state=None) 
    ##### PREDICTION & EVALUATION
    pd.DataFrame(te_data).to_csv('te_data.csv')
    ...
    risk_line = []
    for i in range(int(len(pred[0][0]))):
        if i%10==0:
            risk = np.sum(pred[:,:,:(i+1)], axis=2)
            risk_line.append(risk[0])
    pd.DataFrame(pred[:][0][:]).to_csv('pred.csv')
    pd.DataFrame(risk_line).to_csv('Prediction_risk.csv')
